...
class European_Call_BS(Black_Shcoles_Base_Model.BSBaseModel):
    ...
    def get_theta(self):
        theta=-np.exp(-self.div*self.T)*(self.S*stats.norm.cdf(self.d1,0,1)*self.sigma)/(2*np.sqrt(self.T))-self.r*self.K\
              *np.exp(-self.r*self.T)*stats.norm.cdf(self.d2,0,1)+self.div*self.S*np.exp(-self.div*self.T)*stats.norm.cdf(
              self.d1,0,1)
        return theta
    ...
    def get_vega(self):
        vega=1/np.sqrt(2*np.pi)*self.S*np.exp(-self.div*self.T)*np.exp(-self.d1**2*0.5)*np.sqrt(self.T)
        return vega
